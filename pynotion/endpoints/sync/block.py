...
class BlockSyncEndPoint:
    ...
    def append_block_children(
        ...
        data = self._client.patch(url, body=body)
        return dump_rx("BlockPagination", data)

    def retrieve_block(self, /, block_id: UUID4) -> "RxBlock":
        """
        ...
        """
        data = self._client.get(f"/blocks/{block_id}")
    ...
    def retrieve_block_children(
        ...
        data = self._client.get(f"/blocks/{block_id}/children", params=params)
        return dump_rx("BlockPagination", data)

    def update_block(self, /, block_id: UUID4, block: "TxBlock") -> "RxBlock":
        """
        ...
        body = dump_tx(block)
        data = self._client.patch(f"/blocks/{block_id}", body=body)
        return self._dump_rx_block(data)

    def delete_block(self, /, block_id: UUID4) -> "RxBlock":
        """
        ...
        """
        data = self._client.delete(f"/blocks/{block_id}")
        return self._dump_rx_block(data)

    def restore_block(self, /, block_id: UUID4) -> "RxBlock":
        """
