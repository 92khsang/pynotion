...
class PageAsyncEndPoint:
    def __init__(self, client: NotionAsyncClient):
        self._client = client

    async def create_page(self, page: "TxPage") -> "RxPage":
        """
        ...
        """
        body = dump_tx(page)
        data = await self._client.post("/pages", body=body)
        return dump_rx("RxPage", data)

    async def retrieve_page(self, page_id: UUID4) -> "RxPage":
        """
        ...
        """
        data = await self._client.get(f"/pages/{page_id}")
        return dump_rx("RxPage", data)
    ...
    async def update_page(
        ...
        data = await self._client.patch(f"/pages/{page_id}", body=body)
        return dump_rx("RxPage", data)

    async def delete_page(self, page_id: UUID4) -> "RxPage":
        """
        ...
        """
        data = await self._client.patch(f"/pages/{page_id}", body={"archived": True})
        return dump_rx("RxPage", data)

    async def restore_page(self, page_id: UUID4) -> "RxPage":
        """
        ...
        """
        data = await self._client.patch(f"/pages/{page_id}", body={"archived": False})
        return dump_rx("RxPage", data)
